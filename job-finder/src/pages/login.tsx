...
const Login: React.FC<LoginProps> = ({ onClose, onSwitchToSignup }) => {
  ...
  return (
    <div className="fixed inset-0 flex justify-center items-center z-50">
      <div className="bg-white p-8 rounded shadow-lg max-w-md w-full relative lg:mb-36 mb-64 border border-[#408d24]">
        <button className="absolute top-0 right-0 mt-4 mr-4 text-gray-800" onClick={onClose}>
          &times;
        </button>
        <h2 className="text-2xl font-bold mb-4 mt-2 flex justify-center text-gray-800">LOGIN</h2>
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className='flex justify-center'>
            <button className="bg-[#408d24] text-white border-2  shadow-md px-24 py-2 rounded hover:bg-[#2a6e3e]" type="submit">Login</button>
          </div>
        </form>
